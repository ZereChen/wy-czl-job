...
//    }
    @Test
    public void testFindProductsForSeller(){
        ProdQueryReqForLogin prodQueryReqForLogin = new ProdQueryReqForLogin();
        prodQueryReqForLogin.setNumPerPage(16);
        prodQueryReqForLogin.setPage(1);
        Result<List<ProductEntity>>  productEntityList=productService.findProductsForSeller(prodQueryReqForLogin);
        Assert.assertEquals(new Integer(2),productEntityList.getPages());
        Assert.assertEquals(16,productEntityList.getData().size());
    }
    @Test
